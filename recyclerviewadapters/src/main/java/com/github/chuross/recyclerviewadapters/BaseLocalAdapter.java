...
public abstract class BaseLocalAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> implements LocalAdapter<VH> {
    ...
    private WeakReference<CombinableRecyclerAdapter> parentAdapter;
    ...
    @Override
    public CombinableRecyclerAdapter getParentAdapter() {
        return hasParentAdapter() ? parentAdapter.get() : null;
    }

    @Override
    public void bindParentAdapter(@Nullable CombinableRecyclerAdapter adapter) {
        if (hasParentAdapter()) {
